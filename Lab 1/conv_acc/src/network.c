...
//                        acc += (in_val - conv1_input_zp) * w_val;
						pass_params(conv1_input[0][out_h + f_h][out_w + f_w][0],
								conv1_weights[out_c][f_h][f_w][0], idx, 0);
						w_sum += conv1_weights[out_c][f_h][f_w][0];
						idx++;
                    }
                }

                // Hardware Pipeline: Bias -> Scale -> ZeroPoint -> Activation
				int acc = get_ans(0) - w_sum * conv1_input_zp;
                acc += bias_val;
...
//                            acc += (in_val - conv2_input_zp) * w_val;
							pass_params(
									conv1_output[0][in_h_start + f_h][in_w_start
											+ f_w][in_c],
									conv2_weights[out_c][f_h][f_w][in_c], idx,
									6);
							w_sum += conv2_weights[out_c][f_h][f_w][in_c];
							idx++;
                        }
                    }
                }
                ...
                // Hardware Pipeline: Bias -> Scale -> ZeroPoint -> Activation
				int acc = get_ans(6) - w_sum * conv2_input_zp;
                acc += bias_val;
                acc = multiply_by_quantized_multiplier(acc, m_quant, shift);
                acc += conv2_output_zp;
